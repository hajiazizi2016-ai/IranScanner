...
def detect_psiphon_proxy(timeout=5):
    try:
        ...
        pids = {line.split()[1] for line in tasklist.splitlines() if "psiphon" in line.lower() and len(line.split()) >= 2}
        if not pids:
            return None
...
class TelegramNotifier:
    ...
    def __init__(self, token, chat_id, proxy=None):
        if not token or not chat_id: raise ValueError("Telegram token/chat_id تنظیم نشده.")
        self.token=token; self.chat_id=chat_id; self.proxies={"http":proxy,"https":proxy} if proxy else None
        ...
        for chunk in chunks:
            try:
                ...
                if resp.status_code!=200:
                    logging.error("TELEGRAM ERROR: %s %s",resp.status_code,resp.text); ok=False
            except Exception as e:
                logging.error("TELEGRAM SEND FAILED: %s",e,exc_info=True); ok=False
        return ok
    ...
    def send_file(self,file_path,caption=""):
        ...
        try:
            ...
            if resp.status_code!=200:
                logging.error("TELEGRAM FILE ERROR: %s %s",resp.status_code,resp.text); return False
            return True
        except Exception as e:
            logging.error("TELEGRAM FILE SEND FAILED: %s",e,exc_info=True); return False

    def format_results(self,results,limit=20):
        if not results: return "امروز هیچ سیگنالی پیدا نشد. 🔍"
        lines=[f"📊 <b>نتایج اسکن امروز</b> — {len(results)} سیگنال\n"]
        for s in results[:limit]:
            symbol=getattr(s,"symbol",""); score=round(getattr(s,"final_score",getattr(s,"final_rank",0)),1); rr=round(getattr(s,"rr",0),2); rr_power=round(getattr(s,"rr_power",0),2); vol_ratio=round(getattr(s,"volume_ratio",0),2); buyer_power=round(getattr(s,"buyer_power",0),2); reason=getattr(s,"smart_money_reason","") or getattr(s,"validation_reason","")
            lines.append(f"🔹 <b>{symbol}</b> | امتیاز: {score}\n   RR: {rr} | قدرت RR: {rr_power} | حجم: {vol_ratio}x | قدرت خریدار: {buyer_power}\n   {reason}")
        if len(results)>limit: lines.append(f"\n... و {len(results)-limit} سیگنال دیگر (فایل CSV پیوست است)")
        return "\n".join(lines)

    def send_results(self,results,limit=20): return self.send_message(self.format_results(results,limit=limit))
